...
class GoEnv(Env):
    ...
    def reset(self, initdata = None):
        ...
        self.round = 0
        self.board = b = [[0 for i in range(10)] for j in range(10)]
        for i in range(10):
    ...
    def step(self):
        ...
        if x == y == -1:
            ...
            if self.last_action['x'] == -1:
                ...
                winner = 0 if ax - ay > 0.75 else 1
                self.round = None
                self.display.append({'winner' : winner, 'stones' : {'0' : ax, '1' : ay, 'komi' : 0.75}})
